export default function DarkModeSwitcher({ darkMode, setDarkMode }: { darkMode: boolean, setDarkMode: (value: boolean) => void }) {
	return (
		<div className={`w-20 h-10 p-1 absolute top-5 right-5 sm:right-10 rounded-full cursor-gra transition-all duration-300 ${darkMode ? 'bg-gray-700' : 'bg-white'}`} onClick={() => setDarkMode(!darkMode)}>
