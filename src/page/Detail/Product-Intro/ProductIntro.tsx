...
export default function ProductIntro() {
	return (
		...
		<div id='product-intro' className="w-screen h-screen relative snap-end pt-20">
			...
			<div className="w-full absolute left-1/2 top-[calc(50%+60px)] -translate-x-1/2 -translate-y-1/2 grid grid-rows-3 md:gap-10 sm:grid-rows-1 sm:grid-cols-3 select-none">
				{productNaviInfo.map((item, index) => (
					<a key={index} className="w-full flex flex-col justify-center items-center group btn-scale animation-show" onClick={() => document.getElementById(item.href)!.scrollIntoView({ behavior: 'smooth' })}>
